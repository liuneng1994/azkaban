...
public class FlowRunnerManager implements EventListener,
    ...
    public void submitFlow(HttpServletRequest req, int execId) throws ExecutorManagerException {
        ...
        // 添加扩展参数
        flow.setExtra(getParam(req, "extra", ""));
        logger.info(String.format("submitFlow的extra参数%%s%s", flow.getExtra()));
        if (flow == null) {
        ...
        JdbcDispWfJobParamsLoader dispWfJobParamsLoader = new JdbcDispWfJobParamsLoader(azkabanProps);
        JdbcDispExecutionParamsLoader dispExecutionParamsLoader = new JdbcDispExecutionParamsLoader(azkabanProps);
        FlowRunner runner =
                new FlowRunner(flow, executorLoader, projectLoader, jobtypeManager, dispWfJobParamsLoader, dispExecutionParamsLoader);
